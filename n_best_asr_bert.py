...
from utils.bert_xlnet_inputs import prepare_inputs_for_roberta, prepare_mask_input
...
def parse_arguments():
    ...
    parser.add_argument('--add_info', type=str, default='', help='different setting')
    ...
    parser.add_argument('--add_corpus_token', type=int, default=0, help='tokenizer add token')
    ...
    parser.add_argument('--warm_up', type=int, default=0, help='father pretrain phone embedding')
    ...
    parser.add_argument('--phone_weight', type=float, default=0, help='phone weight')
...
def train_epoch(model, data, opt, memory):
    ...
    epoch_iterator = tqdm(data, desc="Iteration", disable=0)
    for step, batch in enumerate(epoch_iterator):
        ...
        top_scores, bottom_scores_dict, batch_preds, \
        asr_hidden_rep, trans_hidden_rep= model(opt, input_ids, trans_input_ids, seg_ids=seg_input_ids,
                                                          trans_seg_ids=trans_seg_input_ids, classifier_input_type="asr")
...
def warm_epoch(model, data, opt, memory):
    '''Epoch operation in training phase'''
    ...
    epoch_iterator = tqdm(data, desc="WarmUpIteration", disable=0)
    for step, batch in enumerate(epoch_iterator):
        # prepare data
        batch_labels, raw_in, raw_trans_in, raw_labels = batch
        ...
        # prepare inputs for BERT/XLNET
        inputs = {}
        input_ids, seg_input_ids, input_lens, lm_masks, lm_labels = prepare_mask_input(raw_in, opt.tokenizer,
                                                                                       opt, device=opt.device)
        # trans_input_ids, trans_seg_input_ids,\
        # trans_input_lens, trans_lm_masks = prepare_mask_input(raw_trans_in, opt.tokenizer, opt, device=opt.device)

        # forward
        if not opt.add_segment_ids:
            seg_input_ids = None

        _, _, _, lm_log_softmax, _ = model(opt, input_ids, None, seg_ids=seg_input_ids, trans_seg_ids=None,
                                           classifier_input_type="warm_up")

        # backward
        lm_masks = torch.BoolTensor(lm_masks).cuda()
        lm_labels = torch.LongTensor(lm_labels).cuda()
        lm_masks = lm_masks.reshape(-1)
        lm_log_softmax = lm_log_softmax.reshape(-1, len(opt.tokenizer))
        total_loss = -lm_log_softmax[lm_masks].gather(dim=-1, index=lm_labels.unsqueeze(1)).mean()
        ...
        # calculate performance
        pred_label = lm_log_softmax[lm_masks].argmax(dim=-1)
...
def train(model, train_dataloader, valid_dataloader, test_dataloader, opt, memory):
    ...
    model.bert_encoder.resize_token_embeddings(len(opt.tokenizer))
    if opt.warm_up > 0:
        for i in range(opt.warm_up):
            start = time.time()
            warm_loss = warm_epoch(model, train_dataloader, opt, memory)
            logger.info('[Warm]\tEpoch: %02d\tTime: %.2f\tLoss: %.2f' % (i, time.time()-start, warm_loss))
    ...
    for i in range(opt.max_epoch):
        ...
        # logger.info('dont have phone word:{}'.format(', '.join(model.bert_encoder.embeddings.not_in_phone_words)))
...
def add_phone_dict(memory, data_root):
    phone_dict = {}
    file_path = os.path.join(data_root, 'cmudict_SPHINX_40')
    all_phone_tokens = []
    with open(file_path, 'r', encoding='utf-8') as phone_fn:
        for line in phone_fn:
            line = line.rstrip('\n')
            word, phone_str = line.split('\t')
            word = word.lower()
            phone_lst = phone_str.split(' ')
            phone_dict[word] = phone_lst
            for phone in phone_lst:
                if phone not in all_phone_tokens:
                    all_phone_tokens.append(phone)

    phone2idx = {k:v for v, k in enumerate(all_phone_tokens)}
    idx2phone = {k:v for k, v in enumerate(all_phone_tokens)}

    memory['phone_dict'] = phone_dict
    memory['phone2idx'] = phone2idx
    memory['idx2phone'] = idx2phone
    return memory
...
if __name__ == '__main__':
    ...
    # add phone dict
    if opt.use_phone:
        memory = add_phone_dict(memory, opt.dataroot)

    # add special token
    if opt.add_corpus_token:
        special_word_lst = []
        for idx, word in memory['idx2word'].items():
            if idx <= 4: continue
            word = word.lower().replace('?', '')
            if word.isdigit():
                continue
            if opt.tokenizer.tokenize(word) != [word]:
                special_word_lst.append(word)
        special_tokens_dict = {'additional_special_tokens': special_word_lst}
        opt.tokenizer.add_special_tokens(special_tokens_dict)
    ...
    if opt.optim_choice == 'adam':
        ...
        num_train_optimization_steps = (len(train_dataloader.dataset) // opt.batchSize + 1) * (opt.max_epoch + opt.warm_up)
