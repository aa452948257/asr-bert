...
def read_wcn_data(fn, coverage=None, mode='train'):
    '''
    ...
    * system act <=> utterance <=> labels
    ...
    with open(fn, 'r') as fp:
        lines = fp.readlines()
        for line in lines:
            snr_inp, lbl = line.strip('\n\r').split(' <==> ')
            snr_inp_list = ['[cls]'] + list(snr_inp.replace('[cls] ', ''))
            snr_inp_seqs.append(snr_inp_list)
            if len(lbl) == 0:
                labels.append([])
                continue
            if mode == 'train':
                ...
                final_label = []
                for _label in lbl:
                    slot_name = _label.split('-')[1]
                    if slot_name in ['IP', 'Frequency', 'Location']:
                        final_label.append('-'.join(_label.split('-')[:-1]))
                    else:
                        final_label.append(_label)
    ...
    return snr_inp_seqs, snr_inp_seqs, labels
