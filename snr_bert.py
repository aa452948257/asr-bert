...
from utils.dataset.snr_utils import read_wcn_data, prepare_wcn_dataloader, observability_lens, EpochInfoCollector
from utils.gpu_selection import auto_select_gpu
from utils.bert_xlnet_inputs import prepare_inputs_for_bert_chinese
...
def parse_arguments():
    ...
    parser.add_argument('--valid_file', default='10_test', help='base file name of valid dataset')
    parser.add_argument('--test_file', default='10_test', help='base file name of test dataset')
...
def train_epoch(model, data, opt, memory):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        inputs = {}
        input_ids, seg_input_ids, input_lens = \
            prepare_inputs_for_bert_chinese(raw_in, opt.tokenizer, opt, device=opt.device)
        trans_input_ids, trans_seg_input_ids, trans_input_lens = \
            prepare_inputs_for_bert_chinese(raw_trans_in, opt.tokenizer, opt, device=opt.device)
        # forward
        if not opt.add_segment_ids:
            seg_input_ids = None

        top_scores, bottom_scores_dict, batch_preds, asr_hidden_rep, trans_hidden_rep = \
            model(opt, input_ids, trans_input_ids, seg_ids=seg_input_ids, trans_seg_ids=trans_seg_input_ids,
                  classifier_input_type="asr")
        # backward
        loss_record, total_loss = cal_total_loss(top_scores, bottom_scores_dict, batch_preds, batch_labels, memory,
                                                 opt, asr_hidden_rep, trans_hidden_rep)
...
def test_epoch(model, data, opt, memory, fp, efp):
    model.eval()
    ...
    for j, batch in enumerate(data):
        ...
        input_ids,seg_input_ids,input_lens=prepare_inputs_for_bert_chinese(raw_in,opt.tokenizer,opt,device=opt.device)
        trans_input_ids,trans_seg_input_ids,trans_input_lens=prepare_inputs_for_bert_chinese(raw_trans_in,opt.tokenizer,opt,device=opt.device)
        # forward
        if not opt.add_segment_ids:
            seg_input_ids = None
        ...
        for i, (ts, gold, raw) in enumerate(zip(top_scores.tolist(), raw_labels, raw_in)):
            ...
            if opt.ontology is not None:
                pred_classes = filter_informative(new_pred_classes, opt.ontology)
                gold = filter_informative(gold, opt.ontology)

            TP, FP, FN = update_f1(new_pred_classes, gold, TP, FP, FN)
            if set(new_pred_classes) != set(gold):
                print('sentence:{}, pred:{}, gold:{}'.format(sentence, new_pred_classes, gold))

            tot += 1
            if set(new_pred_classes) == set(gold):
                corr += 1

            batch_pred_classes.append(new_pred_classes)
            ...
            res_info = '%s\t<=>\t%s\t<=>\t%s\n' % (
                ' '.join(raw), ';'.join(new_pred_classes), ';'.join(gold))
            fp.write(res_info)
            if set(new_pred_classes) != set(gold):
                efp.write(res_info)
                err_cases.append((raw, new_pred_classes, gold))
            all_cases.append((raw, new_pred_classes, gold))

            # noting inputs, labels and predictions
            raw_inputs.append(" ".join(raw))
            whole_pred_classes.append(new_pred_classes)
            true_golds.append(gold)
            matches.append(True if set(new_pred_classes) == set(gold) else False)
...
def test(model, test_dataloader, opt, memory):
    '''Start testing'''
    ...
    with open(os.path.join(opt.exp_dir, 'test.eval'), 'w') as fp, \
        ...
        start = time.time()
        test_loss, (tep, ter, tef), te_acc, test_all_cases, _ = test_epoch(model, test_dataloader, opt, memory, fp, efp)
...
if __name__ == '__main__':
    ...
    train_data = read_wcn_data(os.path.join(opt.dataroot, opt.train_file), opt.coverage, mode='train')
    valid_data = read_wcn_data(os.path.join(opt.dataroot, opt.valid_file), mode='val')
    ...
    if opt.testing:
        ...
        test_data = read_wcn_data(os.path.join(opt.dataroot, opt.test_file), mode='test')
        test_dataloader = prepare_wcn_dataloader(test_data, memory, int(opt.batchSize / opt.n_accum_steps),
                                                 opt.max_seq_len, opt.device, shuffle_flag=False)
        test(model, test_dataloader, opt, memory)
