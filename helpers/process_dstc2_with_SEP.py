...
def filter_sentence(sentence):
    ...
    sentence = sentence.replace('i\'m', 'am').replace('what\'s', 'what is').replace('don\'t', 'dont'). \
        replace('doesn\'t', 'doesnt').replace('i\'d', 'i would').replace('that\'s', 'that is').replace('won\'t', 'will not'). \
        replace('can\'t', 'cannot').replace('I\'m', 'am').replace('?', ' ? ').replace('you\'ll', 'you will')
...
def read_wcn_data_and_save(log_fn, label_fn, save_fp, bin_norm=False, rm_null=False,augment=True,add_transcription=True,upsample_count=3):
    ...
    for turn, turn2 in zip(log_turns, label_turns):
        ...
        #User input ASR hypothesis separated by [SEP] token
        in_seq = ' [SEP] '.join(batch_nbest)

        prev_sys_response = turn['output']['transcript']

        # in_seq = '[CLS] [SYS] ' + prev_sys_response + ' [USR] ' + in_seq
        in_seq = '[CLS] [SYS] ' + prev_sys_response + ' [USR] ' + batch_1best
        ...
        for asr_hyp in batch_nbest:
            asr_hyp = asr_hyp

            prev_sys_response = filter_sentence(prev_sys_response)
        ...
        in_seq = filter_sentence(in_seq)

        transcription_in_seq = filter_sentence(transcription_in_seq)
        # write sequence
...
processed_data_dir = os.path.join(opt.out_dir, 'processed_data_1best')
...
build_vocab_and_save(all_wcn_words, train_labels, train_sa, memory_fn, min_freq=1)

value_word = []
for word in all_labels:
    value = word.split('-')[-1]
    for _value in value.split(' '):
        if _value in value_word:
            continue
        else:
            value_word.append(_value)

with open('word.dic','w',encoding='utf-8') as word_dict_fn:
    word_dict_fn.writelines([i + '\n' for i in value_word])
word_dict_fn.close()

intent_slot_value = sorted(list(all_labels))
with open('../../BART-SLU/data/dstc2/intent-slot-value.dic', 'w', encoding='utf-8') as label_fn:
    label_fn.writelines([i + '\n' for i in intent_slot_value])
label_fn.close()
